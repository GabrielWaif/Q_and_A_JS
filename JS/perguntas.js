...
function MostrarPergunta() {
  document.querySelector("#perguntal").innerHTML =
    "PERGUNTA " + perguntaAtualID;
  ...
  for (let i = 0; i < perguntas[perguntaAtual].opcoes.length; i++) {
    ...
    <li>
        ...
        <label for="opt${numeroResposta - 1}" id="respostaLabel${
      numeroResposta - 1
    }" class="unselectable">${numeroResposta}. ${
  ...
  checkBoxes.forEach(function (caixa, index) {
    if (caixa.checked) {
      if (index == perguntas[perguntaAtual].indexCerto) {
        acertos++;
        document.querySelector("#respostaLabel" + index).style.backgroundColor =
          "green";
      } else {
        erros++;
        document.querySelector("#respostaLabel" + index).style.backgroundColor =
        ...
        document.querySelector(
          "#respostaLabel" + perguntas[perguntaAtual].indexCerto
